...
class PositionDistanceCalculator:
    ...
    def calculate_distances(
        ...
        PositionDistanceCalculator.calculate_euclidean_distances_pairwise(
            encoded_positions, output_file
        )
        PositionDistanceCalculator.calculate_cosine_similarity_pairwise(
            encoded_positions, output_file
        )
    ...
    @staticmethod
    def calculate_euclidean_distances_pairwise(encoded_position_sets, output_file):
    ...
    def average_euclidean_distance(position_set_a, position_set_b):
        ...
        if position_set_a is position_set_b:
            distances = PositionDistanceCalculator.remove_self_comparisons(distances)

        avg_dist = np.mean(distances)
        return avg_dist

    @staticmethod
    def calculate_cosine_similarity_pairwise(encoded_position_sets, output_file):
        ...
        for i, position_set_a in enumerate(encoded_position_sets):
            for j, position_set_b in enumerate(encoded_position_sets):
                ...
                mean_distance_between_a_and_b = (
                    PositionDistanceCalculator.average_cosine_distance(
                        position_set_a[1], position_set_b[1]
                    )
                )
                result_text = f"Mean cosine similarity between {position_set_a[0]} and {position_set_b[0]} is: {mean_distance_between_a_and_b}\n"
                print(result_text)
                output_file.write(result_text)

    @staticmethod
    def average_cosine_distance(position_set_a, position_set_b):

        set_a_reshaped = position_set_a.reshape(position_set_a.shape[0], -1)
        set_b_reshaped = position_set_b.reshape(position_set_b.shape[0], -1)

        similarities = cosine_similarity(set_a_reshaped, set_b_reshaped)

        if position_set_a is position_set_b:
            similarities = PositionDistanceCalculator.remove_self_comparisons(
                similarities
            )
        return np.mean(similarities)

    @staticmethod
    def remove_self_comparisons(result_matrix):
        mask = np.ones(result_matrix.shape, dtype=bool)
